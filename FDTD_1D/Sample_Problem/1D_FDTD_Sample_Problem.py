...
N_device = 4                                        # Device's smallest structure resolution
structure_min = 0.3048                              # Smallest structure is 1ft = 0.3048m
...
#FFT_Kernel = np.exp(-1j*2*np.pi*dt*Freq)
#REF = np.zeros((time_steps+1,N_freq))
#TRN = np.zeros((time_steps+1,N_freq))
#SRC = np.zeros((time_steps+1,N_freq))
...
for t in range(0,time_steps,1):
    ...
    #for nf in range(0,N_freq,1):
    #    REF[t+1,nf] = REF[t,nf] + (FFT_Kernel[nf]**t)*Ey[t+1,0]
    #    TRN[t+1,nf] = TRN[t,nf] + (FFT_Kernel[nf]**t)*Ey[t+1,-1]
    #    SRC[t+1,nf] = SRC[t,nf] + (FFT_Kernel[nf]**t)*E_source[t]
